...
export async function ensureOpportunityInStage({
  ...
  try {
    ...
    const normalisedHints = nameHints.map(h => String(h || '').trim().toLowerCase()).filter(Boolean);
    if (!pipe) pipe = pipes.find(p => normalisedHints.includes(String(p.name || '').trim().toLowerCase()));
    if (!pipe) pipe = pipes.find(p => normalisedHints.some(h => String(p.name || '').toLowerCase().includes(h)));
    if (!pipe) return null;

...
export function makeInvoiceReference(row) {
  ...
  return `JOB ${row.jobNo} - BPOINT ${identifier}`.slice(0, 255);
}
...
export function makeInvoiceDescription(row) {
  ...
  return `${service} - Job ${row.jobNo} - BPOINT Ref ${row.bpointRef}`.slice(0, 4000);
}
...
function proofPayload(row) {
  return JSON.stringify([
    ...
    row.productDescription, row.description, money(row.amount), row.accountCode,

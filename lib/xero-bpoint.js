...
export const SMOKE_RECEIVE_MONEY_BANK_ACCOUNT_NAME = String(
  process.env.XERO_BPOINT_BANK_ACCOUNT_NAME || 'Business Trans Acct',
).trim();
...
const FIELD_ALIASES = Object.freeze({
  ...
  bankFeedGroup: ['Bank Feed Group', 'Settlement Group', 'Deposit Group'],
...
function normaliseProductDescription(value) {
  return cleanText(value, 1200)
    .replace(/^(?:(?:deposit|instalment|installment)(?:\s+payment)?\s+\d+|final\s+payment)\s+towards\s+/i, '')
    .trim();
}

function normaliseBankFeedGroup(value) {
  const raw = cleanText(value, 40).toUpperCase();
  if (!raw || ['BPOINT', 'BPOINT MIS', 'CBA', 'CBA MIS'].includes(raw)) return '';
  if (['AMEX', 'AMERICAN EXPRESS'].includes(raw)) return 'AMEX';
  throw new Error('Bank Feed Group must be blank/BPOINT or AMEX');
}

export function makeInvoiceReference(row) {
  if (row.category === 'Smoke Alarm' && !row.jobNo) {
    const groupSuffix = row.bankFeedGroup ? `-${row.bankFeedGroup}` : '';
    return `BPOINT-SMOKE-${row.settlementDate.replaceAll('-', '')}${groupSuffix}`;
  }
...
export function makeInvoiceDescription(row) {
  if (row.category === 'Smoke Alarm' && !row.jobNo) {
    ...
    const service = cleanText(row.productDescription || 'Supply and installation of smoke alarms', 4000)
      .replace(/(?:\s*-\s*(?:BPOINT\s+|AMEX\s+)?batch\s+\d{1,2}\/\d{1,2}\/\d{4})+\s*$/i, '')
      .trim();
    const groupLabel = row.bankFeedGroup ? `${row.bankFeedGroup} ` : '';
    return `${service} - ${groupLabel}batch ${day}/${month}/${year}`.slice(0, 4000);
  }
  const service = serviceDescription(row);
...
export function normaliseBatchRow(source, rowNumber = 1) {
  ...
  try {
    ...
    const row = {
      ...
      bankFeedGroup: normaliseBankFeedGroup(rowValue(source, FIELD_ALIASES.bankFeedGroup)),
      ...
      productDescription: normaliseProductDescription(rowValue(source, FIELD_ALIASES.productDescription)),
    ...
    if (category !== 'Smoke Alarm' && row.bankFeedGroup) throw new Error('Bank Feed Group is only available for Smoke Alarm rows');
...
function paymentFromRow(row) {
  return {
    ...
    bankFeedGroup: row.bankFeedGroup,
    amount: row.amount,
...
export function groupInvoiceRows(rows) {
  ...
  for (const row of rows) {
    const key = row.category === 'Smoke Alarm' && !row.jobNo
      ? `SMOKE|${row.settlementDate}|${row.bankFeedGroup}`
      : `JOB|${row.jobNo}`;
...
function proofPayload(row) {
  return JSON.stringify([
    ...
    row.bankFeedGroup, row.productDescription, row.description, money(row.amount), row.invoiceTotal === null ? null : money(row.invoiceTotal), row.accountCode,
    row.division, row.taxType, row.invoiceReference,
    (row.payments || []).map((payment) => [payment.bpointRef, payment.receiptNumber, payment.transactionNumber, payment.paymentDate, payment.settlementDate, payment.bankFeedGroup, money(payment.amount)]),
...
function buildAddress(row, addressType) {
  return {
    addressType,
...
function buildStreetAddress(row) {
  return buildAddress(row, 'STREET');
}

function buildPostalAddress(row) {
  return buildAddress(row, 'POBOX');
}

function primaryPersonName(row) {
  if (!row.jobNo) return {};
  const parts = cleanText(row.customerName).split(' ').filter(Boolean);
  if (parts.length < 2) return parts[0] ? { firstName: parts[0] } : {};
  return {
...
export function buildContact(row) {
  const primaryPerson = primaryPersonName(row);
  const contact = {
  ...
  if (row.address || row.suburb || row.postcode) {
    contact.addresses = [buildStreetAddress(row), buildPostalAddress(row)];
  }
  return contact;
}
...
export function buildContactUpdate(contact, row) {
  const update = {};
  const primaryPerson = primaryPersonName(row);
  if (primaryPerson.firstName && !cleanText(contact.firstName)) update.firstName = primaryPerson.firstName;
  if (primaryPerson.lastName && !cleanText(contact.lastName)) update.lastName = primaryPerson.lastName;
  ...
  if (row.address || row.suburb || row.postcode) {
    ...
    const completedAddresses = [...existingAddresses];
    for (const addressType of ['STREET', 'POBOX']) {
      const addressIndex = completedAddresses.findIndex((address) =>
        String(address.addressType || '').toUpperCase() === addressType);
      if (addressIndex === -1) {
        completedAddresses.push(buildAddress(row, addressType));
        continue;
      }
      const current = completedAddresses[addressIndex];
      const completed = {
        ...current,
      ...
      completedAddresses[addressIndex] = completed;
    }
    if (JSON.stringify(completedAddresses) !== JSON.stringify(existingAddresses)) {
      update.addresses = completedAddresses;
    }
  }
  return update;
}
...
export async function findOrCreateContact(client, row, plannedContact = null) {
  ...
  if (plan.action === 'match') {
    ...
    const idempotencyKey = `goldsure-bpoint-contact-update-v3-${row.contactKey}`.slice(0, 128);
...
function isSmokeReceiveMoney(row) {
  return row.category === 'Smoke Alarm' && !row.jobNo;
}
...
export async function findInvoiceForRow(client, row) {
  if (isSmokeReceiveMoney(row)) {
    const reference = row.invoiceReference;
    const where = `Reference==\"${cleanWhereValue(reference)}\"`;
    const response = await client.accountingApi.getBankTransactions(
      '', undefined, where, undefined, 1, 4, 100, [reference],
    );
    const matches = (response.body.bankTransactions || []).filter((transaction) =>
      cleanText(transaction.reference) === reference
      && String(transaction.type || '').toUpperCase() === 'RECEIVE');
    if (matches.length > 1) throw new Error(`More than one Xero Receive Money transaction uses reference ${reference}`);
    if (!matches[0]) return null;
    return {
      ...matches[0],
      invoiceID: matches[0].bankTransactionID,
      invoiceNumber: 'Receive Money',
      transactionKind: 'receive-money',
    };
  }
...
export function buildApprovedReceiveMoney(row, contactId, bankAccountId) {
  if (!isSmokeReceiveMoney(row)) throw new Error('Receive Money is only available for grouped Smoke Alarm rows');
  if (!contactId) throw new Error('Xero contact ID is missing');
  if (!bankAccountId) throw new Error('Xero bank account ID is missing');
  return {
    type: 'RECEIVE',
    contact: { contactID: contactId },
...
export async function ensureXeroSetup(client, rows) {
  ...
  const activeAccountList = (accountsResponse.body.accounts || [])
    .filter((account) => String(account.status || '').toUpperCase() === 'ACTIVE');
  const activeAccounts = new Map(activeAccountList
  ...
  let receiveMoneyBankAccount = null;
  if (rows.some(isSmokeReceiveMoney)) {
    const bankMatches = activeAccountList.filter((account) =>
      String(account.type || '').toUpperCase() === 'BANK'
      && normaliseName(account.name) === normaliseName(SMOKE_RECEIVE_MONEY_BANK_ACCOUNT_NAME));
    if (bankMatches.length !== 1 || !bankMatches[0].accountID) {
      throw new Error(`Active Xero bank account ${SMOKE_RECEIVE_MONEY_BANK_ACCOUNT_NAME} was not found uniquely`);
    }
    receiveMoneyBankAccount = {
      accountID: bankMatches[0].accountID,
      code: bankMatches[0].code || '',
      name: bankMatches[0].name || '',
    };
  }
  return {
    accounts,
...
export async function previewRowsInXero(client, rows) {
  ...
  for (const row of rows) {
    ...
    plans.push({
      ...
      transactionKind: isSmokeReceiveMoney(row) ? 'receive-money' : 'invoice',
    });
  }
...
export async function createOrFindApprovedInvoice(client, row) {
  const setup = await ensureXeroSetup(client, [row]);
  if (!row.invoiceTotal) throw new Error(`Enter the final invoice amount for Job ${row.jobNo}`);
  ...
  if (existing) {
    ...
    return {
      ...
      transactionKind: existing.transactionKind || 'invoice',
      transactionId: existing.bankTransactionID || existing.invoiceID,
    };
  }

  const contactResult = await findOrCreateContact(client, row);
  if (isSmokeReceiveMoney(row)) {
    const receiveMoney = buildApprovedReceiveMoney(
      row,
      contactResult.contact.contactID,
      setup.receiveMoneyBankAccount?.accountID,
    );
    const idempotencyKey = `goldsure-bpoint-receive-money-${row.rowKey}`.slice(0, 128);
    const response = await client.accountingApi.createBankTransactions(
      '', { bankTransactions: [receiveMoney] }, false, 4, idempotencyKey,
    );
    ...
    return {
      invoiceId: created.bankTransactionID,
      invoiceNumber: 'Receive Money',
      ...
      transactionKind: 'receive-money',
      transactionId: created.bankTransactionID,
      bankAccountName: setup.receiveMoneyBankAccount?.name || SMOKE_RECEIVE_MONEY_BANK_ACCOUNT_NAME,
    };
  }
  ...
  return {
    ...
    transactionId: created.invoiceID,
  };
}

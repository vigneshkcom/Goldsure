...
function itemQuantity(job, key) {
  return (job.items || []).filter(item => item.key === key).reduce((sum, item) => sum + Number(item.quantity || 0), 0);
}

function jobDescription(job) {
  const parts = [
    shortInstalledDate(job.installedDate),
    numericJobNumber(job.jobId),
    itemQuantity(job, 'booking') ? `Booking ${itemQuantity(job, 'booking')}` : '',
    itemQuantity(job, 'hardwired') ? `Hardwired ${itemQuantity(job, 'hardwired')}` : '',
    itemQuantity(job, 'battery') ? `Battery ${itemQuantity(job, 'battery')}` : '',
    itemQuantity(job, 'remote') ? `Controller ${itemQuantity(job, 'remote')}` : '',
  ];
...
export function buildDraftBill(po, contactId, setup) {
  ...
  for (const job of po.jobs || []) {
    ...
    if (Number(job.cashOffset) > 0) {
      const compactJob = `${shortInstalledDate(job.installedDate)} | ${numericJobNumber(job.jobId)}`;
      lineItems.push(billLine(`${compactJob} | Cash offset`, -Number(job.cashOffset), setup, 'NONE'));
    }
  }

...
function quantity(po, job, key) {
  return (job.items || []).filter(item => item.key === key).reduce((sum, item) => sum + Number(item.quantity || 0), 0);
}

function jobDescription(po, job) {
  const parts = [
    `Job ${cleanText(job.jobId, 40)}`,
    cleanText(job.installedDate, 30),
    quantity(po, job, 'booking') ? `Booking ${quantity(po, job, 'booking')}` : '',
    quantity(po, job, 'hardwired') ? `Hardwired ${quantity(po, job, 'hardwired')}` : '',
    quantity(po, job, 'battery') ? `Battery ${quantity(po, job, 'battery')}` : '',
    quantity(po, job, 'remote') ? `Remote ${quantity(po, job, 'remote')}` : '',
  ].filter(Boolean);
...
export async function ensurePurchaseOrderSetup(client, po) {
  ...
  if (normalise(account.name) !== normalise(CONTRACTOR_ACCOUNT_NAME)) {
...
export function buildDraftBill(po, contactId, setup) {
  ...
  for (const job of po.jobs || []) {
    lineItems.push(billLine(jobDescription(po, job), job.grossIncGst, setup));
    if (Number(job.cashOffset) > 0) {
      lineItems.push(billLine(`Cash collected from customer | Job ${cleanText(job.jobId, 40)}`, -Number(job.cashOffset), setup, 'NONE'));
    }
  }

...
async function loadAcceptedQuote(quoteToken, quoteTable, fetchImpl, env) {
...
export async function syncAcceptedQuoteStage({
  quoteToken,
  quoteTable,
  pipelineNames,
  pipelineId = '',
  ...
  const verified = await loadAcceptedQuote(quoteToken, quoteTable, fetchImpl, env);
  ...
  if (!targetPipeline) return { moved: false, reason: 'pipeline-not-found' };
  ...
  if (!opportunity?.id) return { moved: false, reason: 'opportunity-not-found' };
...
export function syncAcceptedSmokeAlarmStage({ quoteToken, fetchImpl = fetch, env = process.env }) {

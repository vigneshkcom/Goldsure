...
const LOGO_URL = 'https://portal.goldsure.com.au/assets/gs-favicon-96px.png';
...
async function renderDocument(po, { title, jobColumnLabel = 'Earnings inc GST', includeInvoiceNote = false } = {}) {
  ...
  doc.rect(36, 36, doc.page.width - 72, 78).fill('#111111');
  let titleX = 46;
  try {
    const logoBuffer = await loadLogoBuffer();
    doc.image(logoBuffer, 46, 46, { width: 30, height: 30 });
    titleX = 84;
  } catch {
    // Logo fetch failed (e.g. offline test run) — fall back to text-only header.
  }
  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(17).text(title, titleX, 47);
  doc.fillColor('#c8aa66').font('Helvetica').fontSize(9).text('Goldsure Pty Ltd', titleX, 69);
  doc.fillColor('#bbbbbb').font('Helvetica').fontSize(8).text(`Issued ${po.issueDate || ''}`, 0, 49, { align: 'right', width: doc.page.width - 46 });
  if (po.poNumber) {
    doc.fillColor('#ffffff').font('Helvetica').fontSize(9).text(po.poNumber, 46, 92, { width: doc.page.width - 82 });
  }

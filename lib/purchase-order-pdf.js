...
async function renderDocument(po, { title, jobColumnLabel = 'Earnings inc GST', includeInvoiceNote = false } = {}) {
  ...
  try {
    ...
    doc.image(logoBuffer, 46, 49, { width: 34, height: 34 });
    titleX = 90;
  } catch {
    // Logo fetch failed (e.g. offline test run) — fall back to text-only header.
  }
  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(18).text(title, titleX, 52);
  doc.fillColor('#c8aa66').font('Helvetica').fontSize(9).text('Goldsure Pty Ltd', titleX, 74);

...
async function renderDocument(po, { title, recipientLabel, amountLabel, finalLabel, showReference, includeInvoiceNote, summaryLayout = false }) {
  ...
  jobs.forEach((job, index) => {
    const jobRowHeight = summaryLayout ? Math.max(rowHeight, Math.ceil(addressHeight(doc, job.address, addressWidth) + 10)) : rowHeight;
    ...
    if (summaryLayout) {
      drawRow(doc, columns, left, y, jobRowHeight, [
        ...
        job.address ? { text: job.address, wrap: true } : { text: '–', color: COLORS.faint },
